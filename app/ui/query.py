...
def render() -> None:
    ...
    st.write("Ask questions about the curated knowledge graph. The MVP uses keyword search while LLM orchestration is wired up.")
...
def _format_query_result(result: QueryResult) -> str:
    ...
    if result.matches:
        ...
        for match in result.matches:
            ...
            lines.append(f"• {match.subject} — {match.predicate} — {match.object}{doc_part}{page_part}")
    if result.note:
        lines.append(f"\n_{result.note}_")
    return "\n".join(lines)
